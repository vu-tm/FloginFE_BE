...
//Test full integration backend Login API, chi mock MockMvc de dam bao su tuong tac giua cac layer
@SpringBootTest
...
public class AuthControllerIntegrationTest {
    ...
    void testLoginFailure_usernameNotFound() throws Exception {
        LoginRequest request = new LoginRequest("testusername1", "Test@123");
    ...
    void testLoginFailure_wrongPassword() throws Exception {
        LoginRequest request = new LoginRequest("testuser", "Test@456");
    ...
    void testLoginFailure_passwordOnlyHasLetters() throws Exception {
        LoginRequest request = new LoginRequest("testuser", "Testeawefaefasdfa");
        ...
        mockMvc.perform(post(LOGIN_API)
                    ...
                    .andExpect(jsonPath("$.success").value(false))
                    .andExpect(jsonPath("$.token").doesNotExist())
                    .andExpect(jsonPath("$.message").value("Password phai chua it nhat 1 chu cai hoa, 1 chu cai thuong, 1 so va 1 ki tu dac biet va khong chua khoang trang"))
                    .andExpect(jsonPath("$.user").doesNotExist());
    }
    ...
    @Test
    @DisplayName("CORS Test Preflight AllowedOrigin voi method khac POST")
    void testPreflightAllowedOriginNotAllowedMethods() throws Exception {
       mockMvc.perform(options("/api/login")
                .header(HttpHeaders.ORIGIN, ALLOWED_ORIGIN)
